...
class QGNN_KGC: # QGNN for knowledge graph completion, i.e., link prediction
    ...
    def train_and_eval(self):
        ...
        if self.encoder.lower() == "qgnn" or self.encoder.lower() == "gcn":
            ...
            print("Training with DistMult only")
            model = DistMult(self.emb_dim, len(self.entity_idxs), len(self.relation_idxs)).to(device)

        print("Using Adam optimizer")
        opt = torch.optim.Adam(model.parameters(), lr=self.learning_rate)

        lst_ents = torch.LongTensor([i for i in range(len(d.entities))]).to(device)
        ...
        for it in range(1, self.num_iterations + 1):
            ...
            if it > self.eval_after and it % self.eval_step == 0:
                print("Validation:")
                tmp_hit10, _, _, _, _ = self.evaluate(model, d.valid_data, lst_ents)
                if max_valid_h10 < tmp_hit10:
                    max_valid_h10 = tmp_hit10
                    best_epoch = it
                    print("Test:")
                    final_test_h10, final_test_h3, final_test_h1, final_test_mr, final_test_mrr = self.evaluate(model, d.test_data, lst_ents)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num_iterations", type=int, default=4000, nargs="?", help="Number of iterations.")
    parser.add_argument("--batch_size", type=int, default=1024, nargs="?", help="Batch size.")
    parser.add_argument("--lr", type=float, default=0.01, nargs="?", help="Learning rate.")
